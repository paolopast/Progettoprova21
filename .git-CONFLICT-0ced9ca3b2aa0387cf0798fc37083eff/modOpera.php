...
<div class = "menuContentBox">
   <!--Contenitore per il menu con i relativi link per le altre pagine del sito-->
	...
	<?php
		...
		$row = $res->fetch_assoc();
    	$str = "<div class = 'titlebox'>Modifica informazioni dell'opera:<br/> ".$row['nome']."</div>";
		echo $str;
        if(isset($_GET['err']) === true)
		{
			$str = "<font color='#FFFF00'> Inserisci i campi obbligatori, stronzo</font>";
			echo $str;
		}
		$str ="<form enctype='multipart/form-data' name = 'modify' action = 'saveModOpera.php?id=".$_GET['id']."' method = 'POST'>
    	<label>Nome:</label><br/>
   		<input name = 'nome' value = '".$row['nome']."'/><br/><br/>
    	<label>Anno:</label><br/>
    	<input name = 'anno' value = '".$row['anno']."'/><br/><br/>
		<label>Autore:</label><br/>
    	<input name = 'autore' value = '".$row['autore']."'/><br/><br/>
        <label>Proprietario:</label><br/>
    	<input name = 'proprietario' value = '".$row['proprietario']."' /><br/><br/>
		<label>Numero Passaporto:</label><br/>
    	<input name = 'numPass' value = '".$row['numPass']."'/><br/><br/>
		<label>Location:</label><br/>
    	<input name = 'location' value = '".$row['location']."' /><br/><br/>
		<label>Materiale:</label><br/>
    	<input name = 'materiale' value = '".$row['materiale']."'/><br/><br/>
		<label>Categoria:</label><br/>
   		<input name = 'categoria' value = '".$row['categoria']."'/><br/><br/>
    	<label>Data ricezione:</label><br/>
    	<input name = 'dataRicezione' value = '".$row['dataRicezione']."'/><br/><br/>
		<label>Tecnica:</label><br/>
    	<input name = 'tecnica' value = '".$row['tecnica']."'/><br/><br/>
        <label>Periodo storico:</label><br/>
    	<input name = 'periodoStorico' value = '".$row['periodoStorico']."' /><br/><br/>
		<label>Dimensioni:</label><br/>
    	<input name = 'dimensioni' value = '".$row['dimensioni']."'/><br/><br/>
		<label>Peso(in chilogrammi)*:</label><br/>
    	<input name = 'peso' value = '".$row['peso']."' /><br/><br/>
		<label>Valore(in euro)*:</label><br/>
    	<input name = 'valore' value = '".$row['valore']."'/><br/><br/>
		<label>Ditta consegna:</label><br/>
   		<input name = 'dittaConsegna' value = '".$row['dittaConsegna']."'/><br/><br/>
		<td>
    	...
    	<input name = 'luogoOrigine' value = '".$row['luogoOrigine']."'/><br/><br/>
        <label>Info storiche:</label><br/>
    	<input name = 'infoStoriche' value = '".$row['infoStoriche']."' /><br/><br/>
		<label>Descrizione:</label><br/>
    	<input name = 'descrizione' value = '".$row['descrizione']."'/><br/><br/>
		<label>Restauri effettuati:</label><br/>
    	<input name = 'restauriEffettuati' value = '".$row['restauriEffettuati']."' /><br/><br/>
		<label>Data pubblicazione:</label><br/>
    	<input name = 'dataPubblicazione' value = '".$row['dataPubblicazione']."'/><br/><br/>
		<td>Sede*:</td>
		<td>
			<select name='sede_id'>";
			echo $str;
		...
		<label>URL video:</label><br/>
   		<input name = 'urlVideo' value = '".$row['urlVideo']."'/><br/><br/>
		...
		$str = "<br/><a href='qrcodegen.php?id=".$_GET['id']."'><button class='action bluebtn'><span class='label'><b>Stampa QR-code opera</b></span></button>";
		echo $str;
		?>
