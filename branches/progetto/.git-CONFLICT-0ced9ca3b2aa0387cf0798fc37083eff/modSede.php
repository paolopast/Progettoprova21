...
<div class = "menuContentBox">
   <!--Contenitore per il menu con i relativi link per le altre pagine del sito-->
	...
	<?php
		...
		$row = $res->fetch_assoc();
    	$str = "<div class = 'titlebox'>Modifica la sede: ".$row['nome']."</div>";
		echo $str;
        if(isset($_GET['err'])===true)
			echo "Inserisci i valori obbligatori";
		$str =" 
    	<form name = 'modify' action = 'saveModSede.php?id=".$_GET['id']."' method = 'POST'>
    	<label>Nome*:</label><br/>
   		<input name = 'nome' rows 1 value = '".$row['nome']."'><br/><br/>
    	<label>Citt&#224:</label><br/>
    	<input name = 'citta' rows = 1 value ='".$row['citta']."'><br/><br/>
        <label>Indirizzo*:</label></br>
    	<input name = 'indirizzo' value = '".$row['indirizzo']."'><br/><br/>
        <label>Numero telefono*:</label><br/>
    	<input name = 'numTelefono' value = '".$row['numTelefono']."'><br/><br/>	
		<button type='submit'>Salva Modifiche</button>

    	</form>		
			<br/><button onclick ='cancella()' class='action bluebtn'><span class='label'>Cancella evento</span></button>";
			echo $str;
		?>
    </div>
</div>
