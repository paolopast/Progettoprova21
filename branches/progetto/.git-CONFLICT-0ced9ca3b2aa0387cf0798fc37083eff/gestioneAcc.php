   ...
   <!--Contenitore per il menu con i relativi link per le altre pagine del sito-->
	...
	<?php
		...
		if($res === false)
			echo mysql_error();
		$row = $res->fetch_assoc();
    	$str = "<div class = 'titlebox'>Gestione Account</div>";
		echo $str;
        if(isset($_GET['err']) === true)
			echo "Inserisci i valori obbligatori";
		$str=" In questa pagina potrai modificare il tuo indirizzo email o la tua password<br/><br/>
    	<form name = 'modify' action = 'saveModAccount.php' method = 'POST'>
    	<label>E-mail*:</label><br/>
   		<input name = 'email' rows 1 value = '".$row['email']."'><br/><br/>
